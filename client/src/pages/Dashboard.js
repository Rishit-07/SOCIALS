...
const Dashboard = () => {
    ...
    const removeFromActiveChallenges = useCallback((challengeId) => {
        setUserChallenges((prev) =>
            prev.filter((challenge) => String(challenge._id) !== String(challengeId))
        );
    }, []);
    ...
    useEffect(() => {
        ...
        const interval = window.setInterval(() => {
            const nextValue = greetingText
                ...
                .map((char, index) => {
                    if (char === ' ') return ' ';
                    if (index < step) return greetingText[index];
    ...
    const confirmLeaveChallenge = async () => {
        if (!challengeToLeave?._id) return;
                ...
                >
                    ...
                    >
                        <div style={{
                            ...
                            marginBottom: '0.85rem', fontSize: '1.25rem',
                        }}>
                            ⚠
                        </div>
                        ...
                        {leaveError && (
                            <p style={{ color: '#ef4444', fontSize: '0.84rem', marginBottom: '0.9rem' }}>{leaveError}</p>
                        )}
                        <div style={{ display: 'flex', gap: '0.65rem', justifyContent: 'flex-end' }}>
                            <motion.button
                                ...
                                style={{
                                    ...
                                    borderRadius: '9px', color: 'rgba(255,255,255,0.85)',
            ...
            {showLoadingScreen && (
                ...
                >
                    ...
                    <motion.p
                        ...
                        style={{ color: '#fff', fontSize: '1rem', fontWeight: 500, letterSpacing: '2px' }}
                    >
                        Loading...
                    </motion.p>
